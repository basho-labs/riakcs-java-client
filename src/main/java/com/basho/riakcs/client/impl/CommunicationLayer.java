...
public class CommunicationLayer {
	...
	public CommunicationLayer(String csAccessKey, String csSecretKey, String csEndpoint, boolean useSSL,
	...
	public HttpURLConnection makeCall(HttpMethod method, URL url, InputStream dataInputStream,
			Map<String, String> headers) throws Exception {
		return makeCallImpl(method, url, true, dataInputStream, headers);
	}

	public HttpURLConnection makeUnsignedCall(HttpMethod method, URL url, InputStream dataInputStream,
			Map<String, String> headers) throws Exception {
		return makeCallImpl(method, url, false, dataInputStream, headers);
	}

	private HttpURLConnection makeCallImpl(HttpMethod method, URL url, boolean isSigned, InputStream dataInputStream,
			Map<String, String> headers) throws Exception {
	...
	private void debugResponse(HttpURLConnection conn) throws Exception {
		...
		/*
		 * We cannot print the response body here in the Java implementation as the HTTP response's input stream cannot
		 * be reset, and therefore cannot be read multiple times. Instead, the response body will be printed by the
		 * methods below that read the response input stream into a String or Document.
		 */
	}
	...
	private String generateRestSignature(HttpMethod method, URL url, Map<String, String> headers) throws Exception {
		...
		if (url.getQuery() != null) {
			for (String param : url.getQuery().split("&")) {
				if (param.equals("acl") || param.equals("torrent") || param.equals("logging")
